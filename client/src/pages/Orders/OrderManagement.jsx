          ...
          <div className="tab-actions">
            <button className="btn-icon" onClick={() => setShowFilterModal(true)}>
              ...
              {(dateFilter.start ||
                ...
                paymentFilter.length > 0) && <span className="filter-badge">•</span>}
            </button>
          </div>
            ...
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort("orderID")} style={{ cursor: "pointer" }}>
                  Order ID{" "}
                  {sortConfig.key === "orderID" && (sortConfig.direction === "asc" ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
                </th>
                <th>Product</th>
                <th onClick={() => handleSort("orderDate")} style={{ cursor: "pointer" }}>
                  Date{" "}
                  {sortConfig.key === "orderDate" && (sortConfig.direction === "asc" ? <ChevronUp size={14} /> : <ChevronDown size={14} />)}
                </th>
                <th>Customer</th>
              ...
              {paginatedOrders.length === 0 ? (
                ...
                paginatedOrders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td>
                      <span className={`status-badge ${getStatusClass(order.orderStatus)}`}>{displayEnumValue(order.orderStatus)}</span>
                    </td>
                    <td>
              ...
              <div className="modal-body">
                <div className="order-details-grid">
                  ...
                  <div className="detail-item">
                    ...
                    <p>{viewOrder.orderCustomer?.customerName || "N/A"}</p>
                  </div>
                  <div className="detail-item">
                    <label>Email</label>
                  ...
                  <div className="detail-item">
                    <label>Total</label>
                    <p>₹{viewOrder.OrderedProduct?.price?.toFixed(2) || "0.00"}</p>
                  </div>
                  <div className="detail-item">
                    <label>Payment</label>
                  ...
                  <div className="detail-item">
                    <label>Status</label>
                    <p className={`status-badge ${getStatusClass(viewOrder.orderStatus)}`}>{displayEnumValue(viewOrder.orderStatus)}</p>
                  </div>
                  <div className="detail-item full-width">
                    <label>Address</label>
                    <p>{viewOrder.orderCustomer?.customerAddress || "N/A"}</p>
                  </div>
                  {viewOrder.orderType && (
                    <div className="detail-item">
          ...
          <div className="modal-overlay" onClick={() => { setShowAddModal(false); setSearchedTerm(""); }}>
            <div className="modal-content modal-add-order" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                <div className="order-details-form">
                  <div className="form-group full-width">
                    ...
                    {isNewCustomer ? (
                      <div className="form-grid">
                        ...
                        <div className="form-group full-width">
                          <button className="btn-primary" onClick={createCustomer} disabled={loading} style={{ display: "flex", alignItems: "center", gap: "8px", justifyContent: "center" }}>
                  ...
                  {newOrder.prodID && (
                    <div className="selected-product">
                      ...
                      <div className="product-info-mini">
                        <div className="product-name">{products.find((p) => p.id === parseInt(newOrder.prodID))?.productName || "Selected Product"}</div>
                      </div>
                      <div className="product-price">₹{products.find((p) => p.id === parseInt(newOrder.prodID))?.price || "0"}</div>
                    </div>
                  )}
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      <label>Order Date</label>
                      <input type="date" value={newOrder.orderDate} onChange={(e) => setNewOrder({ ...newOrder, orderDate: e.target.value })} placeholder={new Date().toISOString().split("T")[0]} />
                    </div>
                    <div className="form-group">
                      <label>Order Time</label>
              ...
              <div className="modal-footer modal-footer-add">
                <button className="btn-cancel-outline" onClick={() => setShowAddModal(false)}>
                  Cancel
                </button>
                <button
        ...
        {showDeleteModal && (
          <div className="modal-overlay" onClick={() => setShowDeleteModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                <p>
                  Are you sure you want to delete {orderToDelete?.id === "multiple" ? `${orderToDelete.count} orders` : `order ${orderToDelete?.orderID}`}?
                </p>
        ...
        {showFilterModal && (
          <div className="modal-overlay" onClick={() => setShowFilterModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-footer">
                <button className="btn-cancel" onClick={clearFilters}>
                  Clear All
                </button>
